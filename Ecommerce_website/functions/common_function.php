...
function search_product() {
    ...
    $search_query = "Select * from `products` where product_keywords like '%$search_data_value%'";
                ...
                $num_of_rows = mysqli_num_rows($result_query);
                if($num_of_rows <= 0) {
                    echo "<h2 class='text-center text-danger'>No Result Matchs. No product found on this Category! </h2>";
                }
                 while($row = mysqli_fetch_assoc($result_query)) {
                    $product_id = $row['product_id'];
                    $product_title =$row['product_title'];
...
function cart() {
    if(isset($_GET['add_to_cart'])) {
        ...
        $num_of_rows = mysqli_num_rows($result_query);
        if($num_of_rows > 0) {
            echo "<script class='text-center text-danger'>alert(`This item already present inside cart`);</script>";
            echo "<script>window.open('index.php','_self')</script>";
        }
        else {
            $insert_query = "insert into `cart_details` (product_id , ip_address, quantity) values ($get_product_id, '$get_ip_add',0)";
            $result_query= mysqli_query($con,$insert_query);
            echo "<script class='text-center text-danger'>alert(`Item is added to cart`);</script>";
            echo "<script>window.open('index.php','_self')</script>";
        }
    }
}
